...
The key entries for this dataset are `AreaWithConcern` (and `Area`), which express a geographic region paired with a point-of-interest category. Areas are listed **from smallest to largest** — the first token is the innermost (most specific) area, and each subsequent token is a larger containing area.
...
Understanding OSM's `name:en` conventions is important for both data generation and
inference-time fallback strategies. Three patterns were identified:
...
The two-call fallback strategy (→ [RF-008](docs/research_findings/RF-008-two-call-name-fallback.md)) handles Pattern A and B by retrying with `["name"=...]` when `["name:en"=...]` returns zero results.

**Japanese ward naming** differs by city type:
- Tokyo's 23 special wards (特別区): `name:en` **without** "Ward" suffix — e.g., `"Shinjuku"`, `"Shibuya"`
- Other cities' administrative wards (行政区): `name:en` **with** "Ward" suffix — e.g., `"Chuo Ward"`, `"Atsuta Ward"`
...
- **Administrative hierarchy gaps (Pattern C)**: Some city areas exist in OSM but are not linked into their containing region's relation hierarchy, causing `(area.inner)(area.outer)` to return zero. These are not recoverable by the name fallback.
...
| [RF-009](docs/research_findings/RF-009-v4-multilevel-augmentation.md) | v4 dataset augmentation: 92.0% → 99.1% via multi-level training pairs + system prompt | Confirmed |
...
**Novelty of this project** vs prior work: TRIDENT intermediate language · automated synthetic data generation with Overpass API validation · edge-device fine-tuning target (0.5B on Raspberry Pi 5) · two-call `name:en` → `name` fallback · OSM tagging convention documentation.
...
LoRA fine-tuning code lives in a separate private repository (`text2geoql-ft-poc`) and uses **PEFT + TRL** (no Unsloth). Adapters are evaluated against guaranteed-nonempty pairs verified via the public Overpass API.
