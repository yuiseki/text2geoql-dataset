...
日本語名称を直接 Nominatim に渡す場合、階層表現がなくても解決できる:

| クエリ | 解決先 |
|-------|--------|
| `広島市` | 広島市 (id=4097196) ✓ |
| `広島県` | 広島県 (id=3218753) ✓ |
| `Hiroshima Prefecture` | 広島県 (id=3218753) ✓ |
| `Hiroshima City` (英語 City 付き) | **道路名 (id=157500466) ✗** |

**日本語の「市」「県」「区」等の行政区画接尾辞は Nominatim にとって十分な曖昧性解消の手がかりになる。**

ただし英語で `Hiroshima City` と入力すると OSM に存在する道路名等にマッチしてしまう。
`Hiroshima Prefecture` は英語でも正しく解決できる（"Prefecture" が接尾辞として機能するため）。

## この問題が難しい理由

**接尾辞なしの単一地名**（"広島"・"Hiroshima" のみ）や自然言語クエリ「広島のカフェを教えて」は曖昧である:
- "広島" は広島県を指すか、広島市を指すか
- 多くの場合ユーザー自身も意識していない
...
を同時に必要とする。**GPT-5.1 を含む大型 LLM であっても、曖昧な自由テキスト入力に対してこの問題を安定的に解くことは報告されていない。**
